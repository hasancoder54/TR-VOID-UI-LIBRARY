...
function Library:InitKeySystem(cfg)
    ...
    KeyFrame.ClipsDescendants = true -- Taşmayı önleyen kritik satır
    Instance.new("UICorner", KeyFrame).CornerRadius = UDim.new(0, 12)
    
    -- Background Image (KeyFrame'in içine alındı)
    ...
    GetBtn.MouseButton1Click:Connect(function()
        ...
        Library:Notify("System", "Key link copied!", 3)
    end)
...
function Library:CreateWindow(cfg)
    ...
    local fpsL = CreateStat("FPS: 0", Theme.Text)
    local msL = CreateStat("MS: 0", Theme.Text)
    local exploitL = CreateStat("Exploit: Delta", Theme.Accent)
    ...
    ContainerHolder.Size = UDim2.new(1, -20, 1, -125)
    ...
    function Tabs:CreateTab(name)
        ...
        function Elements:CreateMultiDropdown(text, list, callback)
            ...
            MainBtn.MouseButton1Click:Connect(function()
                open = not open
                TweenService:Create(DropFrame, TweenInfo.new(0.4), {Size = UDim2.new(0.96, 0, 0, open and (45 + (#list * 35) + 5) or 45)}):Play()
            end)
        end

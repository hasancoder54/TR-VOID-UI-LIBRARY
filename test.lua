...
-- [KEY SİSTEMİ - RESİM TAM EKRAN DÜZELTİLDİ]
function Library:InitKeySystem(cfg)
    ...
    KeyFrame.BackgroundColor3 = Theme.Main; KeyFrame.ZIndex = 1; KeyFrame.ClipsDescendants = true
    Instance.new("UICorner", KeyFrame); Instance.new("UIStroke", KeyFrame).Color = Theme.Accent

    -- RESİM TAM KAPLAMA MANTIĞI
    if CustomImage ~= "" then
        local img = Instance.new("ImageLabel", KeyFrame)
        img.Name = "Background"
        img.Size = UDim2.fromScale(1, 1) -- Tüm alanı kapla
        img.Position = UDim2.fromScale(0, 0)
        img.BackgroundTransparency = 1
        img.ScaleType = Enum.ScaleType.Crop -- Resmin bozulmadan sığmasını sağlar
        img.ZIndex = 2
        img.ImageTransparency = 0.2 -- Arka planın hafif siyahlığıyla birleşsin
    ...
    input.BackgroundColor3 = Theme.Element; input.TextColor3 = Theme.Text; input.ZIndex = 3; input.PlaceholderText = "Key Giriniz..."
    Instance.new("UICorner", input)

    local btn = Instance.new("TextButton", KeyFrame)
    btn.Size = UDim2.new(0.8, 0, 0, 45); btn.Position = UDim2.new(0.1, 0, 0.7, 0)
    btn.BackgroundColor3 = Theme.Accent; btn.Text = "DOĞRULA"; btn.ZIndex = 3; btn.TextColor3 = Theme.Text; btn.Font = 3
    Instance.new("UICorner", btn)

    btn.MouseButton1Click:Connect(function()
        if input.Text == CorrectKey then KeyGui:Destroy(); Callback() else Library:Notify("Hata", "Yanlış Key!", 2) end
    end)
end
...
function Library:CreateWindow(cfg)
    ...
    Main.BackgroundColor3 = Theme.Main; Instance.new("UICorner", Main); Instance.new("UIStroke", Main).Color = Theme.Accent
    ...
    function Tabs:CreateTab(name)
        ...
        function Elements:CreateButton(text, cb)
            local b = Instance.new("TextButton", Container); b.Size = UDim2.new(0.95, 0, 0, 35); b.BackgroundColor3 = Theme.Element; b.Text = text; b.TextColor3 = Theme.Text; Instance.new("UICorner", b)
            b.MouseButton1Click:Connect(cb)
        end
        function Elements:CreateToggle(text, cb)
            local state = false; local t = Instance.new("TextButton", Container); t.Size = UDim2.new(0.95, 0, 0, 35); t.BackgroundColor3 = Theme.Element; t.Text = "  " .. text; t.TextColor3 = Theme.Text; t.TextXAlignment = 0; Instance.new("UICorner", t)
            local s = Instance.new("Frame", t); s.Size = UDim2.new(0, 25, 0, 25); s.Position = UDim2.new(1, -30, 0, 5); s.BackgroundColor3 = Theme.Main; Instance.new("UICorner", s)
        ...
        function Elements:CreateLabel(text)
            local l = Instance.new("TextLabel", Container); l.Size = UDim2.new(0.95, 0, 0, 30); l.BackgroundTransparency = 1; l.Text = text; l.TextColor3 = Theme.Accent; l.Font = 3
